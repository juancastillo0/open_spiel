...
#ifndef OPEN_SPIEL_GAMES_DUDO_H_
#define OPEN_SPIEL_GAMES_DUDO_H_
...
class DudoState : public State {
 public:
  explicit DudoState(std::shared_ptr<const Game> game, int total_num_dice,
                          int max_dice_per_player,
                          const std::vector<int>& num_dice);
  DudoState(const DudoState&) = default;
...
class DudoGame : public Game {
 public:
  explicit DudoGame(const GameParameters& params, GameType game_type);
...
#endif  // OPEN_SPIEL_GAMES_DUDO_H_
